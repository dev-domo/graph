...
//        printGraph(matrix);
    ...
    private String dijkstra(int totalNode, int[][] matrix) {
        ...
        for (int i = 0; i <= totalNode; i++){
            ...
            if(u == -1) break; // 더 이상 처리할 노드가 없는 경우 (방문할 정점이 없는 경우) 중단
            visited[u] = true; // 정점 방문 처리. -> 다시 방문하지 않도록 함.

            // 인접 노드들의 거리 업데이트
            for (int v = 0; v < totalNode; v++){
                if (matrix[u][v] > 0 && !visited[v] && distance[u] + matrix[u][v] < distance[v]){
    ...
    private String formatPath(int node, int[] parent) {
        if (parent[node] == -1) return String.valueOf(node + 1); // 현재 노드가 시작점이면 해당 노드를 반환.
